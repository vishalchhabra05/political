                ...
                <div class="col-sm-12 mb-4">
                    <div class="box bg-white comman-form-design">
                        <div class="box-row top-search-table">
                            <a href="<?= route('create_elections') ?>" class="btn btn-primary " style="float: right;">
...
$(function () {
    var table = $('#table_dataTable').DataTable({
        ...
        order: [[5, "desc"]],
        "ajax": {
            "url": '{!! route('elections.datatables') !!}',
            "dataType": "json",
            "type": "POST",
        ...
        columns: [
            ...
            {data: 'election_name', name: 'election_name', orderable:true},
...
function changeStatus(id){
    swal({
        title: `Do you want to change election status?`,
        text: "",
        icon: "warning",
        buttons: true,
        // dangerMode: true,
    ...
    .then((willUpdate) => {
        ...
        if (willUpdate) {
            jQuery.ajax({
                url: '{{route('update_elections_status')}}',
                type: 'POST',
                data:{id:id},
                headers: {
                ...
                success: function (response) {
                    if(response.status == true){
                        var electionTable = $('#table_dataTable').DataTable();
                        electionTable.draw();
